...
本プロジェクトは「完全ローカル対応」です。外部CDNを使用せず、必要なライブラリ・アイコン・フォントはすべて本リポジトリに同梱されています。
...
### 受付フロー
- **予約あり**: 「姓 名（半角スペース）」で予約照合。希望未入力なら選択画面へ遷移し、確認後に確定/待機。
- **予約なし（当日参加）**: 氏名/学校名/学年を入力→希望を選択→確定。体験不参加も選択可（説明会のみ案内）。

### プログラム選択
- 各カードの「第1/第2/第3希望」ボタンで選択。重複不可。満員は選択不可表示。

## 管理画面（歯車アイコン → パスワード `admin`）
### タブ: プログラム編集
- 並べ替え（リストアイコンのドラッグ）、タイトル/説明/定員の編集、プログラム追加、JSON編集。
- 「英語翻訳表示」で `タイトル(英)/説明文(英)` を表示（未入力は既定の英語文で補完）。

### タブ: ファイル読み込み
- 予約者名簿（xlsx）と説明会名簿（xlsx）を読み込み（ローカル保存）。

### タブ: 名簿プレビュー
- 氏名/フリガナ検索、受付状況の色表示（緑=確定、黄=待機、赤=未受付）、列マッピング表示。

### タブ: 受付状況
- プログラムごとの確定/定員・参加者をカード/表で表示。「待機者を一括割り当て」「Excelに書き出し」。

### タブ: 設定
- **予約者優先割り当て**、**学年優先割り当て（予約なし）**、**受付データをリセット**。

## 名簿Excelのフォーマット
- **ミニキャップストーン体験 予約者名簿**: A=No, B=姓, C=名, D=セイ, E=メイ, …, O=第1希望, P=第2希望, Q=第3希望 → `name`（姓+名 半角スペース）, `furigana`, `choices`
- **工学部説明会 参加者名簿**: A=No, B=時間, C=姓, D=名, E=セイ, F=メイ → `name`, `furigana`, `time`

## データ保存仕様（ローカル）
- **IndexedDB** と **localStorage** を併用。
- 自動保存/復元: 入力途中フォーム、受付データ、名簿データ、設定。
- クリア: 管理画面「受付データをリセット」。

## セキュリティとプライバシー
- **CSP（Content-Security-Policy）**: `index.html` に厳格なメタCSPを設定。
  - 許可ソースはすべて `self` のみ（完全ローカル）。
  - 例: `style-src 'self' 'unsafe-inline'`, `font-src 'self' data:`, `img-src 'self' data:`。
- **入力のハードニング**: `autocomplete="off"`, `autocapitalize="off"`, `spellcheck="false"`。
- **XSS対策**: 動的HTMLは `escapeHTML()` でサニタイズ。
- **PII最小化**: 個人情報を含むログは出力せず、データは外部送信しません。

## オフライン運用
- すべての外部依存（フォント/アイコン/ライブラリ）を `assets/` と `vendor/` に同梱。
- `index.html` はローカル資産のみを参照し、ネットワーク不要で動作。

## 出力（エクスポート）
- 「Excelファイルに書き出し」で `reception_status.xlsx` を生成。
  - 列: `プログラム`, `お名前`（英語表示時は英語タイトル）

## ディレクトリ構成
- `index.html`, `style.css`, `script.js`, `public/`, `register_of_names/`
- `assets/fonts/`: `Inter` / `Noto Sans JP` / `Zen Maru Gothic` の woff2 と `fonts.css`
- `vendor/sortable/`: SortableJS（オフライン）
- `vendor/sheetjs/`: SheetJS（`xlsx.full.min.js` オフライン）
- `vendor/phosphor-icons/`: Phosphor Icons（CSS+Webfonts オフライン）

## 既知の注意点
- 本アプリは完全ローカル対応です。表示不具合時は `assets/` / `vendor/` 配下の構成をご確認ください。
- 管理者パスワードは初期値 `admin`。変更は `script.js` 内の該当箇所を編集。
- 氏名は「姓 名」の半角スペースで照合（全角は自動で半角に正規化）。

## ライブラリ
- SortableJS / SheetJS / Phosphor Icons / Inter / Noto Sans JP / Zen Maru Gothic（すべてローカル同梱）

## トラブルシュート
- 予約が見つからない: 名簿未インポート/氏名のスペース・表記ゆれを確認。
- 定員いっぱい: 代替プログラムを案内。後で「待機者を一括割り当て」を使用可。
- レイアウト崩れ/初期化したい: 「受付データをリセット」を実行後、再読み込み。
- アイコンが表示されない: `vendor/phosphor-icons/Fonts/regular/style.css` / `fill/style.css` の読み込み、および `<i class="ph ph-*>` の指定を確認。
