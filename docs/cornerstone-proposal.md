...
gantt
    ...
    section Phase 5：フィードバック改善①
    6月OC フィードバック反映・修正 :         p5a, 2026-07-01, 21d
    総合テスト・負荷テスト        :         p5b, 2026-07-20, 10d
    本番環境セットアップ          :         p5c, 2026-07-28, 7d
    8月初旬 OC 本番稼働①         :milestone, m2, 2026-08-03, 1d

    section Phase 6：フィードバック改善②（夏休み集中開発）
    8月初旬 OC フィードバック反映 :         p6a, 2026-08-05, 14d
    追加機能・改善実装            :         p6b, 2026-08-10, 14d
    8月下旬 OC 本番稼働②         :milestone, m3, 2026-08-25, 1d

    section Phase 7：最終仕上げ
    8月下旬 OC フィードバック反映 :         p7a, 2026-08-27, 10d
    最終調整・ドキュメント整備    :         p7b, 2026-09-05, 7d
    9月中旬 OC 最終稼働（完成版） :milestone, m4, 2026-09-14, 1d
```
...
| フェーズ | 期間 | 目的 |
|---------|------|------|
| **Phase 1–3** | 4月〜6月 | 基盤開発・新機能実装・6月OC でデモ |
| **Phase 4** | 6月〜7月上旬 | タイプ2振り分け・リピーター機能など高度な機能を実装 |
| **Phase 5** | 7月 | **6月OC のフィードバックを受けて修正・改善**。8月初旬OC に向けて本番準備 |
| **Phase 6** | 8月 | 8月初旬OC 本番稼働①。夏休み期間を活用した集中開発で8月下旬OC 本番稼働② |
| **Phase 7** | 8月末〜9月 | 最終フィードバック反映。**9月中旬OC を完成版の最終目標** として仕上げる |
| **その後** | 2027年3月以降 | 翌年のオープンキャンパスに向けた追加改善・機能拡張 |

> **夏休み期間（7月下旬〜8月）は開発時間を十分に確保できる**ため、フィードバックを受けてから次のオープンキャンパスまでの改善サイクルを短く回せることが強みです。
...
| 氏名 | 役割 | 担当範囲 | 主な使用技術 |
|------|------|----------|------------|
| （未定） | **チームリーダー / フルスタック** | 全体設計・進捗管理・結合調整 | React, Node.js, Firebase |
| （未定） | **フロントエンド担当** | UI/UX設計・受付画面・QRスキャナー | React, TypeScript, Tailwind |
| （未定） | **バックエンド担当** | API設計・振り分けアルゴリズム・DB | Node.js, Express, Firestore |
| （未定） | **インフラ・デプロイ担当** | Firebase設定・CI/CD・セキュリティ | Firebase, GitHub Actions |
| （未定） | **UI/UX・ドキュメント担当** | デザイン統一・ドキュメント整備・テスト | Figma, Markdown, Jest |
...
本プロジェクトにはコーナーストーン制度により **5万円 × 5名 = 25万円** の予算が大学から支給されます。精算方式は**後払い・実費清算**であり、使い残した分の返還義務はありません。

| 予算総額 | 支給条件 | 精算方式 |
|---------|---------|---------|
| **250,000円** | 大学による承認後に支給 | 後払い・実費清算（使い残しは返還不要） |

### 費用概算（変動あり）

以下は現時点での概算です。実際の費用は使用量・調達状況によって変動します。

| 項目 | 概要 | 年間概算費用 |
|------|------|------------|
| Firebase Blaze プラン | Cloud Functions・Firestoreの従量課金。OC 1回あたり20〜100名規模のため無料枠内に収まることが多い | 0〜10,000円/年 |
| メール配信（Amazon SES） | QRコード送信メールの配信費用。従量課金制で本規模なら年間ほぼ無料 | 0〜500円/年 |
| 受付端末（iPad等） | 大学や学部から借用できる可能性があるため、必要が生じた場合のみ調達 | 0円（要調整） |
| 開発ツール・ライセンス | デザインツール・有料ライブラリ等の想定 | 〜10,000円 |
| **費用合計（概算）** | | **〜20,500円** |

> **予算と費用の差額（約23万円）** は使用しません。コーナーストーン制度の仕組み上、実際に費用として発生した分のみを後払いで請求します。費用概算は今後の開発・運用状況によって変動する可能性があります。
...
本システムはオープンキャンパスのたびに継続的に使用されることを想定しており、開発チームの卒業後も運用が続く可能性があります。現時点での管理体制の移行については以下の方針を考えています。

| 移行先候補 | 概要 |
|-----------|------|
| **後継学生チーム** | 本プロジェクトを引き継ぐ学生が毎年更新・改善を担当する |
| **工学部OC改善委員会** | 現在オープンキャンパスを運営している委員会に管理を移管する |
| **大学（職員）への寄贈** | システムとドキュメントを大学に引き渡し、職員が管理・運用する |

> **現時点での方針：** 最終的な移行先はプロジェクト終了時点の状況に応じて決定します。いずれの場合も、管理者権限の引き継ぎ手順・運用マニュアルを整備した上で移管します。また Firebase 等の外部サービスのアカウント・APIキーについても、個人アカウントに紐付けず、大学または委員会名義での管理を前提に設計します。
