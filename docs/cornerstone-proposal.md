...
> **精算方式：後払い・実費清算。** 費用は実際に使用した分のみが対象となり、使い残した額は返還不要です。

| 項目 | 用途 | 予算（円） |
|------|------|----------|
| Firebase Blaze プラン | Cloud Functions・Firestore の本番運用費 | 20,000 |
...
#### Firebase ランニングコストについて

本システムのユーザー規模はオープンキャンパス1回あたり **20〜100名程度** を想定しています。Firebase の無料枠（Sparkプラン）はCloud Functionsの呼び出し上限があるため、安定稼働のために Blaze（従量課金）プランへ移行しますが、この規模では月額費用はほぼ無料〜数百円程度に収まる見込みです。上記の2万円は **2026〜2027年の複数回のオープンキャンパスを通じた累計費用の上限見積もり** です。
