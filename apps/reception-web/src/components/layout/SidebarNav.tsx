    ...
    <nav className="flex flex-1 flex-col gap-4">
      {sections.map((section) => (
        <div key={section.title} className="flex flex-col gap-2">
          <h3 className="px-2 text-xs font-semibold uppercase tracking-[0.22em] text-slate-500/80 dark:text-slate-400/80">
            {section.title}
          </h3>
          <div className="flex flex-col gap-1">
            {section.items.map((item) => (
              <button
                ...
                className={clsx(
                  'group inline-flex items-center justify-between gap-3 rounded-xl px-3 py-3 text-sm font-medium transition',
